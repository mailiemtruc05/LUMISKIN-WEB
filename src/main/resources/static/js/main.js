...
$(function(){
  ...
  // đọc discount chỉ 1 lần
  const discount = parseFloat($('.cart-total').data('discount')) || 0;

  function refreshTotals(cartTotal){
    // update Subtotal
    $("#cartSubtotal").text(formatVND(cartTotal));
    if(discount > 0){
      // update Discount (nếu có)
...
// Bắt event click “Xóa mã” để AJAX, chỉ xoá mã giảm giá
$('.remove-coupon').on('click', function(e) {
  e.preventDefault();
  $.get('/shopping-cart/remove-coupon', function() {
    // Xoá đúng nút Xóa mã
    $('.remove-coupon').remove();
    // Xoá luôn dòng hiển thị Discount
    $('#cartDiscount').closest('p.d-flex').remove();
    // Reset Total = Subtotal
    const subText = $('#cartSubtotal').text();
    $('#cartTotal').text(subText);
    // Clear discount trong data để sau này refreshTotals không trừ nữa
